<template>
    <div>
        <input type="checkbox" v-model="isDark" class="light-switch sr-only" id="light-switch" />
        <label
...
const isDark = useDark({
    ...
    onChanged: (dark) => {
        ...
        if (dark) {
            ...
            theme.sweetAlert2('dark')
            theme.flexMonster('dark')
            document.documentElement.classList.add('dark')
        } else {
            localStorage.setItem('theme', 'light')
            theme.sweetAlert2('light')
            theme.flexMonster('light')
